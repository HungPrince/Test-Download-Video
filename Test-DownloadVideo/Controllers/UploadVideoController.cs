...
namespace Test_DownloadVideo.Controllers
{
    ...
    {
        private string videoAddress = "~/Upload/Videos";
        ...
        public string UploadComplete(string fileName, string complete)
        {
            ...
            System.IO.File.Move(Path.Combine(tempPath, fileName), Path.Combine(videoPath, newName + fileType));

            VideoSteam videoVM = new VideoSteam
            {
                FileType = fileType,
                FileName = newName,
                Url = "Upload/Videos/" + newName + fileType
            };

            using (DBEntities db = new DBEntities())
            {
                db.VideoSteams.Add(videoVM);
                db.SaveChanges();
            }
